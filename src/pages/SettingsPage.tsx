...
import { PUBLIC_DEMO_MODE, appAssetPath } from "../core/publicDemo";
import {
 backupSchema,
...
export default function SettingsPage(){
 ...
 const resetAll=async()=>{
  ...
  setFactoryText("");
  await reload();
  window.location.assign(appAssetPath(""));
 };
 ...
 return <div className="app-page settings-page">
  ...
  <main className="settings-scroll"><div className="settings-form">
   ...
   <section className="main-model-section">
    ...
    <details className="provider-compatibility-diagnostics"><summary>兼容性诊断（高级）</summary><p className="section-note">正常使用无需修改。仅在接口文档明确要求特定协议，或需要诊断浏览器跨域问题时展开设置。</p><label>请求协议<select value={form.protocol??"auto"} onChange={event=>set("protocol",event.target.value as ProviderSettings["protocol"])}><option value="auto">自动判断</option><option value="openai-compatible">OpenAI 兼容</option><option value="openai-responses">OpenAI Responses</option><option value="gemini">Gemini 原生</option><option value="claude">Claude 原生</option><option value="deepseek-compatible">DeepSeek 兼容</option></select><small className="section-note">官方 Gemini / Claude API 请选择对应原生协议；中转站若提供 /chat/completions，请选择 OpenAI 兼容。</small></label>{PUBLIC_DEMO_MODE?<p className="section-note">公开演示版固定使用浏览器直连，不提供 Relay 或其他共享后端。</p>:<label>请求通道<select value={form.networkMode??"direct"} onChange={event=>{set("networkMode",event.target.value as ProviderSettings["networkMode"]);setForm(current=>({...current,networkModeExplicit:true}))}}><option value="relay">安全 Relay（高级）</option><option value="direct">浏览器直连（高级）</option></select><small className="section-note">安全 Relay 用于兼容不允许网页跨域访问的 API 中转站，且不会保存 API Key。两个通道之间不会自动重放请求。</small></label>}</details>
   ...
   <SettingsAccordion title="数据与备份" subtitle={PUBLIC_DEMO_MODE?"仅本地备份与恢复":"本地备份与 GitHub 加密备份"}>
