...
export default function MeetSessionPage() {
  ...
  const copyGenerationDiagnostic = async (entry: MeetEntry) => {
    ...
    const attemptLines = (generation.attempts ?? []).flatMap((attempt) => [
      ...
      `attempt${attempt.ordinal}.failureSegmentIndex=${attempt.failureSegmentIndex ?? "none"}`,
      `attempt${attempt.ordinal}.failureSegmentType=${attempt.failureSegmentType ?? "none"}`,
      `attempt${attempt.ordinal}.failureField=${attempt.failureField ?? "none"}`,
      `attempt${attempt.ordinal}.segmentCount=${attempt.segmentCount ?? "unknown"}`,
      ...
      `attempt${attempt.ordinal}.networkMode=${attempt.networkMode ?? "unknown"}`,
      `attempt${attempt.ordinal}.relayUsed=${Boolean(attempt.relayUsed)}`,
      `attempt${attempt.ordinal}.relayRequestId=${attempt.relayRequestId ?? "none"}`,
      `attempt${attempt.ordinal}.relayStatus=${attempt.relayStatus ?? "unknown"}`,
      `attempt${attempt.ordinal}.relayErrorCode=${attempt.relayErrorCode ?? "none"}`,
    ...
    const diagnostic = [
      ...
      "networkMode=" + (generation.networkMode ?? "unknown"),
      "relayUsed=" + Boolean(generation.relayUsed),
      "relayRequestId=" + (generation.relayRequestId ?? "none"),
      "relayStatus=" + (generation.relayStatus ?? "unknown"),
      "relayErrorCode=" + (generation.relayErrorCode ?? "none"),
      "relayDurationMs=" + (generation.relayDurationMs ?? "unknown"),
      "upstreamHttpStatus=" + (generation.upstreamHttpStatus ?? "unknown"),
      "upstreamBytes=" + (generation.upstreamBytes ?? "unknown"),
      ...
      "failureSegmentIndex=" + (generation.failureSegmentIndex ?? "none"),
      "failureSegmentType=" + (generation.failureSegmentType ?? "none"),
      "failureField=" + (generation.failureField ?? "none"),
      "segmentCount=" + (generation.segmentCount ?? "unknown"),
