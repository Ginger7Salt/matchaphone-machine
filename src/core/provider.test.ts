...
describe("OpenAI provider",()=>{ it("sends a valid output token limit and disables streaming when requested",async()=>{let payload:any;vi.stubGlobal("fetch",vi.fn().mockImplementation(async(_url,options:any)=>{payload=JSON.parse(options.body);return new Response(JSON.stringify({choices:[{message:{content:"OK"}}]}),{status:200,headers:{"Content-Type":"application/json"}})}));await new OpenAIProvider(settings).chat([{role:"user",content:"hi"}],{stream:false});expect(payload.max_tokens).toBeGreaterThan(0);expect(payload.stream).toBe(false);expect(payload).not.toHaveProperty("max_completion_tokens")});
